package tower;

import static org.junit.jupiter.api.Assertions.*;
import org.junit.jupiter.api.AfterEach;
import org.junit.jupiter.api.BeforeEach;
import org.junit.jupiter.api.Test;

/**
 * Clase de prueba que tiene como objetivo verificar que el proyecto completo funcione
 * correctamente.
 *
 * @author Juan Pablo Cuervo Contreras
 * @author David Felipe Ortiz Salcedo
 * @version 15/04/2026
 */
public class TowerATest {
    private Tower newTower;

    /**
     * Método llamado antes de cada caso de prueba.
     */
    @BeforeEach
    public void setUp() {
        newTower = new Tower(10, 100);
    } // Cierre de la clase de prueba
    
    /**
     * Caso de prueba que verifica que no se puedan agregar dos tazas del mismo tamaño
     * sin importar que tipo de taza sea.
     */
    @Test
    public void shouldNotAddTwoSameCups() {
        newTower.pushCup("opener", 5);
        newTower.pushCup("hierarchical", 5);
        assertFalse(newTower.ok());
    } // Cierre de la clase de prueba
    
    /**
     * Caso de prueba que comprueba que una taza deba ser tapada.
     */
    @Test
    public void shouldCoverCup() {
    	newTower.pushCup("normal", 5);
    	newTower.pushLid("normal", 5);
    	newTower.cover();
    	assertTrue(newTower.ok());
    } // Cierre del caso de prueba
    
    /**
     * Caso de prueba que verifica que al revertir la torre, las tazas
     * cambien de lugar.
     */
    @Test
    public void shouldReverseTowerWithCups() {
    	newTower.pushCup("normal", 3);
    	newTower.pushCup("normal", 5);
    	newTower.reverseTower();
    	assertTrue(newTower.ok());
    } // Cierre del caso de prueba
    
    /**
     * Caso de prueba que tiene como fin, ordenar la torre de acuerdo
     * a las tapas.
     */
    @Test
    public void shouldOrderTowerWithLids() {
    	newTower.pushLid("normal", 6);
    	newTower.pushLid("normal", 1);
    	newTower.pushLid("normal", 4);
    	newTower.orderTower();
    	assertTrue(newTower.ok());
    } // Cierre del caso de prueba

    /**
     * Método llamado después de cada caso de prueba.
     */
    @AfterEach
    public void tearDown() {
    } // Cierre de la clase de prueba
} // Cierre de la clase