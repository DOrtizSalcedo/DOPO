...
public class Tower {
    ...
    private static final int PIXEL_PER_CM = 10; 
    ...
    public void pushCup(String type, int size) {
        ...
        int yPos = GROUND_Y - (newBaseCm + cupHeightCm) * PIXEL_PER_CM;

        Cup newCup = createCup(type, size, color, CENTER_X, yPos);
    ...
    public void pushLid(String type, int size) {
        ...
        String color = getColorForSize(size);
        int yPos = GROUND_Y - (newBaseCm + lidHeightCm) * PIXEL_PER_CM;
        Lid newLid = createLid(type, size, color, CENTER_X, yPos);
    ...
    private void redraw() {
        ...
        for(int i = 0; i < items.size(); i++){
            ...
            if(item.getType().equals("cup")) {
                Cup cup = (Cup) item;
                int yPos = GROUND_Y - (baseCm + cup.getSize()) * PIXEL_PER_CM;
                cup.moveTo(CENTER_X, yPos);
                cup.makeVisible();
            } else if(item.getType().equals("lid")) {
                Lid lid = (Lid) item;
                int yPos = GROUND_Y - (baseCm + 1) * PIXEL_PER_CM;
                lid.getView().moveTo(CENTER_X - 2, yPos);
    ...
    private void drawRuler() {
        ...
        axis.changeSize(maxHeight * PIXEL_PER_CM, 2);
        
        int rulerX = CENTER_X - 60; 
        int moveX = rulerX - 70;
        int moveY = (GROUND_Y - (maxHeight * PIXEL_PER_CM)) - 15;
