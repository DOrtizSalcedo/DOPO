...
public class Fifa{
    ...
    */
    public void addPlayer(String name, String minutes, String position, String value, String club) throws FifaException { 
        if (name == null || name.trim().isEmpty()) throw new FifaException(FifaException.INVALID_NAME);
        if (consult(name) != null) throw new FifaException(FifaException.PLAYER_ALREADY_EXISTS);
        ...
        if (m < 0) throw new FifaException(FifaException.MINUTES_UNKNOWN);
        if (v < 0) throw new FifaException(FifaException.VALUE_UNKNOWN);

        Player np=new Player(name,m,position.charAt(0),v,club);
        participants.add(np);
    ...
    */
    public void addTeam(String name, String minutes, String position, String manager, String uniform, String thePlayers) throws FifaException { 
        if (name == null || name.trim().isEmpty()) throw new FifaException(FifaException.INVALID_NAME);
        if (consult(name) != null) throw new FifaException(FifaException.CLUB_ALREADY_EXISTS);
        ...
        if (m < 0) throw new FifaException(FifaException.VALUE_UNKNOWN);

        Team c = new Team(name,m,position.charAt(0),manager, uniform);
        String [] aPlayers= thePlayers.split("\n");
        for (String b : aPlayers){
            if (b.trim().isEmpty()) continue;
            Player p = players.get(b.toUpperCase());
            if (p != null) {
                c.addPlayer(p);
            }
        }
