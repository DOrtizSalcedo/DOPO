...
public class TeamTest{
    ...
    // defaultMarkedValue
    @Test
    public void shouldThrowExceptionIfMinutesAndValueAreNegatives() {
        Team t = new Team("COLOMBIA",1620, 'K', "Lorenzo", "Amarill-Rojo-Azul");    
        t.addPlayer(new Player("JAMES", -516,'M',-3500000,"Minnesota"));
        try {
            int valor = t.defaultMarkedValue(3500000, 516);
    ...
    @Test
    public void shouldUseDefaultMinutes() {
        Team t = new Team("COLOMBIA", 1620, 'K', "Lorenzo", "Amarill-Rojo-Azul");    
        t.addPlayer(new Player("BORRE", -110, 'A', 4400000,"Sport Club"));
        try {
            int valor = t.defaultMarkedValue(4400000, 110);
            assertEquals(110, valor);
        } catch (FifaException e) {
            fail("Se lanzo la excepción");
        }
    }
    
    // bestMarkedValue
    @Test
    public void shouldThrowExceptionIfValueUnknown() {
        Team t = new Team("COLOMBIA", 1620, 'K', "Lorenzo", "Amarill-Rojo-Azul");
        t.addPlayer(new Player("JAMES", 80,'M', -2200000, "Minnesota"));
        try {
            int valor = t.bestMarkedValue();
            fail("No se lanzo la excepción");
        } catch (FifaException e) {
            assertEquals(FifaException.VALUE_UNKNOWN, e.getMessage());
        }
    ...
    @Test
    public void shouldReturnBestMarkedValue() {
        Team t = new Team("COLOMBIA",1620, 'K', "Lorenzo", "Amarill-Rojo-Azul");
        t.addPlayer(new Player("L.DIAZ", 690,'A',760000000,"Bayer"));
        t.addPlayer(new Player("BORRE", 445,'A',4400000,"Sport Club"));
        t.addPlayer(new Player("LUCUMI", 1250,'D',125000000,"Bologna"));
